...
const columnOverlay = {
  ...
  addColumn(win, columnId, columnLabel) {
    ...
    let ordinals = [];
    let treecols = threadCols.querySelectorAll("treecol"); // gets every column regardless if active or not
    treecols.forEach((elem) => {
      ordinals.push(elem.ordinal)
    });
    let nextOrdinal = (Math.max(...ordinals) + 1 || "").toString();

    const treeCol = win.document.createXULElement("treecol"); // creates a new treecol 
...
var DDCHeaderView = {
  init(win) {
    console.log("DDCHeaderView init");
    this.win = win;
